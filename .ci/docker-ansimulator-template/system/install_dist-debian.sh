...
if [ ! -z "${PKG_APT_PROXY}" ]; then
    ...
    sed -i "s#=deb-cache.domain.tld#=${PKG_APT_PROXY_IP}#g"  /etc/apt/detect_proxy.sh
    sed -i "s#=3142#=${PKG_APT_PROXY_PORT}#g"  /etc/apt/detect_proxy.sh
